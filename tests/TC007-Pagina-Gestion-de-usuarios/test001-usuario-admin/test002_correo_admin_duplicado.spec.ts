...
test('Correo duplicado', async ({ page }) => {
  ...
  await page.fill('//input[@name="firstName"]', data.Nombre_admin);

  // Ingreso de apellido

  await page.fill('//input[@name="lastName"]', data.Apellido_admin);

  // Ingreso de correo

  await page.fill('//input[@name="email"]', data.Correo_admin);
  ...
  await page.getByPlaceholder('Ingresá el número de cédula').fill(data.Cedula2);
  ...
  const successMessage = page.locator("(//div[contains(.,'¡Existe un usuario con el mismo correo electrónico!')])[4]");
  await successMessage.waitFor({ state: 'visible' });
  
  // Verifica que el mensaje esté visible y tenga el texto esperado
  await expect(successMessage).toBeVisible();
  await expect(successMessage).toHaveText('¡Existe un usuario con el mismo correo electrónico!');
  ...
  const screenshotPath = path.join(carpetaBase, 'correo_duplicado.png');
