...
test('Crear usuario admin', async ({ page }) => {
  ...
  const successMessage = page.locator("(//div[contains(.,'Usuario creado con éxito.')])[4]");
  await successMessage.waitFor({ state: 'visible' });
  
  // Verifica que el mensaje esté visible y tenga el texto esperado
  await expect(successMessage).toBeVisible();
  await expect(successMessage).toHaveText('Usuario creado con éxito.');
  ...
  const screenshotPath = path.join(carpetaBase, 'Crear_usuario_admin.png');
