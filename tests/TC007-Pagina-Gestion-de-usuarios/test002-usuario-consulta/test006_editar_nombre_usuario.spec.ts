...
test('Editar nombre del usuario', async ({ page }) => {
  ...
  await page.getByRole('row', { name: 'Maria Dominguez' }).getByRole('button').click();
  await page.getByRole('button', { name: 'Editar' }).click();

  const input = await page.locator('//input[@name="firstName"]');
  await input.click(); // enfocar
  await input.fill(''); // borrar
  await input.type(data.Nombre_consulta_dos); // escribir nuevo dato
  ...
  const screenshotPath = path.join(carpetaBase, 'editar_nombre_usuario.png');
