...
test('Verificar trx estado error', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Icon Filtrar' }).click();
  await page.getByRole('cell', { name: 'Error' }).locator('span').click();
  ...
  const screenshotPath = path.join(carpetaBase, 'Verificar_trx_estado_error.png');
