...
test('Verificar trx estado denegada', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Denegada' }).click();
  await page.getByRole('button', { name: 'Icon Filtrar' }).click();
  await page.getByText('Denegada').nth(1).click();
  //await page.getByRole('cell', { name: 'Denegada' }).locator('span').click();
  ...
  const screenshotPath = path.join(carpetaBase, 'Verificar_trx_estado_denegada.png');
