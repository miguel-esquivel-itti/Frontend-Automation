...
test('Verificar trx otras entidades', async ({ page }) => {
  ...
  await page.getByPlaceholder('Código de autorización').fill(data.Cod_Autorizacion_Otra_entidades);
  await page.getByRole('button', { name: 'Icon Filtrar' }).click();
  await page.getByRole('img', { name: 'QR logo', exact: true }).click();
  await page.getByRole('button', { name: 'Close' }).click();
  await page.getByPlaceholder('Código de autorización').click();
  await page.getByPlaceholder('Código de autorización').fill('');
  await page.getByPlaceholder('Boleta N°').click();
  await page.getByPlaceholder('Boleta N°').fill(data.Boleta_Otra_entidades);
  await page.getByRole('button', { name: 'Icon Filtrar' }).click();
  await page.getByRole('cell', { name: data.Boleta_Otra_entidades }).first().click();
  ...
  const screenshotPath = path.join(carpetaBase, 'Verificar_trx_otras_entidades.png');
