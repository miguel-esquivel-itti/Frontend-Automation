...
test('Verificar trx upay', async ({ page }) => {
  ...
  await page.getByPlaceholder('Código de autorización').fill(data.Cod_Autorizacion_UPAY);
  await page.getByRole('button', { name: 'Icon Filtrar' }).click();
  ...
  await page.getByPlaceholder('Boleta N°').fill(data.Boleta_UPAY);
  await page.getByRole('button', { name: 'Icon Filtrar' }).click();
  await page.getByRole('cell', { name: data.Boleta_UPAY }).first().click();
  ...
  const screenshotPath = path.join(carpetaBase, 'Verificar_trx_upay.png');
