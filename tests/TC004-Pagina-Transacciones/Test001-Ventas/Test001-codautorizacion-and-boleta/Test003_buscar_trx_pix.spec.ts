...
test('Verificar trx pix', async ({ page }) => {
  ...
  await page.getByPlaceholder('Código de autorización').fill(data.Cod_Autorizacion_PIX);
  await page.getByRole('button', { name: 'Icon Filtrar' }).click();
  ...
  await page.getByPlaceholder('Boleta N°').fill(data.Boleta_PIX);
  await page.getByRole('button', { name: 'Icon Filtrar' }).click();
  await page.getByRole('cell', { name: data.Boleta_PIX }).click();
  ...
  const screenshotPath = path.join(carpetaBase, 'Verificar_trx_pix.png');
