...
test('Verificar trx depay', async ({ page }) => {
  ...
  await page.getByPlaceholder('Código de autorización').fill(data.Cod_Autorizacion_DEPAY);
  await page.getByRole('button', { name: 'Icon Filtrar' }).click();
  await page.getByRole('img', { name: 'Depay logo', exact: true }).click();
  await page.getByRole('button', { name: 'Close' }).click();
  await page.getByPlaceholder('Código de autorización').click();
  await page.getByPlaceholder('Código de autorización').fill('');
  await page.getByPlaceholder('Boleta N°').click();
  await page.getByPlaceholder('Boleta N°').fill(data.Boleta_DEPAY);
  await page.getByRole('button', { name: 'Icon Filtrar' }).click();
  await page.getByRole('cell', { name: data.Boleta_DEPAY }).first().click();
  ...
  const screenshotPath = path.join(carpetaBase, 'Verificar_trx_depay.png');
