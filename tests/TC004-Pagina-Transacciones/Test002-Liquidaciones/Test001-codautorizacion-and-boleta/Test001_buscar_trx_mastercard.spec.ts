...
test('Verificar trx mastercard', async ({ page }) => {
  ...
  await page.getByPlaceholder('Código de autorización').fill(data.liq_Cod_Autorizacion_MC);
  //await page.getByRole('button', { name: 'Icon Filtrar' }).click();
  await page.waitForTimeout(3000); // Espera 2000 milisegundos (2 segundos)
  await page.getByRole('button', { name: 'Marca' }).click();
  await page.getByRole('button', { name: 'Mastercard image Mastercard' }).click();
  await page.getByRole('button', { name: 'Icon Filtrar' }).click();
  await page.getByPlaceholder('Código de autorización').click();
  await page.getByPlaceholder('Código de autorización').fill('');
  await page.getByPlaceholder('Boleta N°').click();
  await page.getByPlaceholder('Boleta N°').fill(data.liq_Boleta_MC);
  await page.getByRole('button', { name: 'Icon Filtrar' }).click();
  //await page.getByRole('cell', { name: data.Boleta_MC }).click();
  //await page.getByRole('button', { name: 'Icon Filtrar' }).click();
  await page.getByRole('img', { name: 'Mastercard logo' }).click();
  ...
  const screenshotPath = path.join(carpetaBase, 'Verificar_trx_mastercard.png');
