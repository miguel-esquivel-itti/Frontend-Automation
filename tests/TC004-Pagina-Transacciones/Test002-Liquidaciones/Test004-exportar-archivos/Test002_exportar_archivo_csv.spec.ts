...
test('Exportar archivo CSV', async ({ page }) => {
  ...
  await page.getByText('Exportado con éxito.').click();
    ...
    const carpetaBase = path.join(__dirname, '..','..','..','screenshots','Pagina transacciones liquidaciones','exportar');  // Usamos __dirname para referirnos a la ubicación actual del test

    // Definir el nombre del archivo de la captura
    const screenshotPath = path.join(carpetaBase, 'exportar_archivo_csv.png');
