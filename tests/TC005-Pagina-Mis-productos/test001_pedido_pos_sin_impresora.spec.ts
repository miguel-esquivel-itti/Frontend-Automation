...
test('Pedido exitoso pos sin impresora', async ({ page }) => {
  ...
  const carpetaBase = path.join(__dirname, '..', 'screenshots', 'Pagina mis productos','Pedido exitoso pos sin impresora');  // Usamos __dirname para referirnos a la ubicación actual del test
  
  // Definir el nombre del archivo de la captura
  const screenshotPath = path.join(carpetaBase, 'pedido_exitoso_pos_sin_impresora.png');
