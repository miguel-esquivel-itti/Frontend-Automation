...
test('Inicio de sesión exitoso rol super admin', async ({ page }) => {
  ...
  await expect(page.locator('//p[contains(.,"Inicio")]')).toBeVisible();
  ...
  // Le indicamos que espere hasta que la pagina contenga 
  await expect(page.locator('//span[contains(.,"Admin")]')).toBeVisible();
    ...
    const screenshotPath = path.join(carpetaBase, 'Inicio_de_sesión_exitoso_rol_super_admin.png');
