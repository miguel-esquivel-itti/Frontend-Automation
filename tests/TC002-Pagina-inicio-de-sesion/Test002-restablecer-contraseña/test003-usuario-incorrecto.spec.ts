...
test('Enviar solicitud de reseteo con un correo no válido', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/olvide-contrasena/);
    ...
    await page.fill('input[name="email"]', data.invalidEmail);
  ...
  await expect(page.locator('text="El correo electrónico no está registrado."')).toBeVisible();
    ...
    const carpetaBase = path.join(__dirname, '..','..', 'screenshots', 'Pagina de inicio de sesion','restablecer contraseña');  // Usamos __dirname para referirnos a la ubicación actual del test

    // Definir el nombre del archivo de la captura
    const screenshotPath = path.join(carpetaBase, 'Enviar_solicitud_de_reseteo_con_un_correo_no_válido.png');
