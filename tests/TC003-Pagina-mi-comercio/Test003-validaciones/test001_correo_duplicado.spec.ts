...
test('Correo duplicado', async ({ page }) => {
  ...
  await page.getByText('El correo electrónico ingresado ya está en uso. Por favor, utiliza uno diferente.', { exact: true }).waitFor({ state: 'visible' });

  // Definir la ruta donde se guardarán las capturas
  const carpetaBase = path.join(__dirname, '..','..', 'screenshots', 'Pagina mi comercio','correo duplicado');  // Usamos __dirname para referirnos a la ubicación actual del test
